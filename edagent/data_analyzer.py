...
Basic Statistics for Numerical Columns:
{
            chr(10).join(
                [
                    f"- {col}: mean={stats.get('mean', 'N/A')}, min={stats.get('min', 'N/A')}, max={
                        stats.get('max', 'N/A')
                    }"
...
Basic Statistics for Categorical Columns:
{
            chr(10).join(
                [
                    f"- {col}: unique_count={stats.get('unique_count', 'N/A')}, most_frequent='{
                        stats.get('most_frequent', 'N/A')
                    }' (appears {stats.get('most_frequent_count', 'N/A')} times)"
                    for col, stats in metadata["categorical_statistics"].items()
                ]
            )
        }
"""
